...
export function getFriendLink() {
  ...
  // Extract host from relay URL (https://example.com -> example.com)
  const relayUrl = relay.getUrl ? relay.getUrl() : relay.RELAY_URL;
  const relayHost = relayUrl.replace(/^https?:\/\//, '');

  return {
    success: true,
    link: `clawlink://${relayHost}/add?${params.toString()}`,
    name: config.displayName
  };
}
...
export async function getStatus() {
  ...
  try {
    ...
    relayStatus = (health.status === 'ok' || health.status === 'healthy') ? 'online' : 'error';
  } catch {
    relayStatus = 'offline';
  }
